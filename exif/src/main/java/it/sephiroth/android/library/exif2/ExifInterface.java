...
public class ExifInterface {
    ...
    private HashMap<Integer, Integer> mTagInfo = null;
    ...
    public void writeExif(final String dstFilename) throws IOException {
        ...
        // System.out.println(TAG+ "delete old backup file" );
        bak_file.delete();

        // rename dst file into backup file
        // System.out.println(TAG+ "rename dst into bak" )
        // if( ! dst_file.renameTo( bak_file ) ) return;

        try {
            // System.out.println(TAG+ "try to write into dst" );
            ...
            // System.out.println(TAG+ "rename the bak into dst" );
            copyFile2(bak_file, dst_file);
			/*boolean rename = bak_file.renameTo(dst_file);
			//remane失败:需要用流对拷
			System.out.println(TAG+ " rename the bak into dst success :"+rename );*/
        } catch (IOException e) {
            e.printStackTrace();
            throw e;
        } finally {
    ...
    public static void copyFile2(File var0, File var1) throws IOException {
        copyStream(new FileInputStream(var0), new FileOutputStream(var1));
    }
    ...
    public void writeExif(final BufferedImage input, final String dstFilename, int quality) throws IOException {
        ...
        // 指定写图片的方式为 jpg
        imgWrier = ImageIO.getImageWritersByFormatName(dstFilename.substring(dstFilename.lastIndexOf(".") + 1)).next();
        imgWriteParams = new javax.imageio.plugins.jpeg.JPEGImageWriteParam(
                null);
        // 要使用压缩，必须指定压缩方式为MODE_EXPLICIT
        imgWriteParams.setCompressionMode(ImageWriteParam.MODE_EXPLICIT);
        // 这里指定压缩的程度，参数qality是取值0~1范围内，
        imgWriteParams.setCompressionQuality(quality * 1.0f / 100);
        imgWriteParams.setProgressiveMode(ImageWriteParam.MODE_DISABLED);
        ColorModel colorModel = input.getColorModel();// ColorModel.getRGBdefault();
        imgWriteParams.setDestinationType(new javax.imageio.ImageTypeSpecifier(
                colorModel, colorModel.createCompatibleSampleModel(32, 32)));


        //todo 支持其他格式
        ImageIO.write(input, "JPEG", out);
    ...
    private static int writeExif_internal(final InputStream input, final OutputStream output, ExifData exifData) throws IOException {
        // System.out.println(TAG+ "writeExif_internal" );
        ...
        if (sections.get(0).type != JpegHeader.TAG_M_JFIF) {
            System.out.println(TAG + "first section is not a JFIF or EXIF tag");
    ...
    protected Map<Integer, Integer> getTagInfo() {
        if (mTagInfo == null) {
            mTagInfo = new HashMap<Integer, Integer>();
            initTagInfo();
        }
        return mTagInfo;
    }
    ...
    public int setTagDefinition(
        ...
        if (ExifTag.isValidType(tagType) && ExifTag.isValidIfd(defaultIfd)) {
            ...
            Map<Integer, Integer> infos = getTagInfo();
    ...
    public BufferedImage getThumbnailBitmap() {
        if (mData.hasCompressedThumbnail()) {
            ...
            try {
                return ImageIO.read(new ByteArrayInputStream(thumb));
            } catch (Exception e) {
                e.printStackTrace();
            }
     ...
     */
    @SuppressWarnings("unused")
    public boolean setCompressedThumbnail(BufferedImage thumb) {
        ByteArrayOutputStream thumbnail = new ByteArrayOutputStream();
        //ImageIO.write(RenderedImage im, String formatName, OutputStream output);
        try {
            ImageIO.write(thumb, "JPEG", thumbnail);
