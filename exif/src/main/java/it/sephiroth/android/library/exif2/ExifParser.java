...
class ExifParser {
    ...
    private ExifParser(InputStream inputStream, int options, ExifInterface iRef) throws IOException, ExifInvalidFormatException {
        ...
        // System.out.println( "sections size: " + mSections.size() );
    ...
    private CountedDataInputStream seekTiffData(InputStream inputStream) throws IOException, ExifInvalidFormatException {
        ...
        if (a != 0xFF || b != JpegHeader.TAG_SOI) {
            System.out.println(TAG + "invalid jpeg header");
            return null;
        }
        ...
        while (true) {
            ...
            if (a > 10) {
                System.out.println("Extraneous " + (a - 1) + " padding bytes before section " + marker);
            }
            ...
            switch (marker) {
                ...
                case JpegHeader.TAG_M_EOI:
                    ...
                    System.out.println("No image in jpeg!");
                    return null;
                ...
                case JpegHeader.TAG_M_EXIF:
                    if (itemlen >= 8) {
                        ...
                        if (header == EXIF_HEADER && headerTail == EXIF_HEADER_TAIL) {
                            ...
                            System.out.println(TAG + "Image cotains XMP section");
                        }
                    }
                    break;

                default:
                    System.out.println("Unknown marker: " + String.format("0x%2X", marker) + ", length: " + itemlen);
                    break;
            }

            if (!ignore) {
                // System.out.println( "adding section with size: " + section.size );
                mSections.add(section);
            } else {
                System.out.println(TAG + "ignoring marker: " + String.format("0x%2X", marker) + ", length: " + itemlen);
            }
        }
    }
        ...
        while (a < data.length) {
            ...
            if (reftable != null) { // terse output includes quality
                ...
                if (tableindex == 0) {
                    ...
                    // System.out.println(TAG+ "quality guess: " + mQualityGuess );
                }
            }
        }
    }
...
//	}
    ...
    protected int next() throws IOException, ExifInvalidFormatException {
        ...
        if (offset < endOfTags) {
            ...
            if (mIfdType == IfdId.TYPE_IFD_0) {
                ...
                if (offsetSize < 4) {
                    System.out.println("Invalid size of link to next IFD: " + offsetSize);
                } else {
                    long ifdOffset = readUnsignedLong();
                    if (ifdOffset != 0) {
                        System.out.println("Invalid link to next IFD: " + ifdOffset);
                    }
                }
            }
        }
        while (mCorrespondingEvent.size() != 0) {
            ...
            try {
                ...
                System.out.println("Failed to skip to data at: " + entry.getKey() +
            ...
            if (event instanceof IfdEvent) {
                ...
                if (mNumOfTagInIfd * TAG_SIZE + mIfdStartOffset + OFFSET_SIZE > mTiffStream.getEnd()) {
                    System.out.println("Invalid size of IFD " + mIfdType);
                    return EVENT_END;
                }
    ...
    private ExifTag readTag() throws IOException, ExifInvalidFormatException {
        ...
        if (!ExifTag.isValidType(dataFormat)) {
            System.out.println(String.format("Tag %04x: Invalid data type %d", tagId, dataFormat));
            mTiffStream.skip(4);
            return null;
        }
    ...
    public boolean isDefinedTag(int ifdId, int tagId) {
        try {
            int tag = ExifInterface.defineTag(ifdId, (short) tagId);
            if (mInterface.getTagInfo().containsKey(tag)) {
                Integer val = mInterface.getTagInfo().get(tag);
                if (val == null) {
                    return false;
                }
                return ExifInterface.DEFINITION_NULL != val;
            }
            //return mInterface.getTagInfo().get(tag) != ExifInterface.DEFINITION_NULL;
            ...
            if (mCorrespondingEvent.size() > 0) {
                if (mCorrespondingEvent.firstEntry().getKey() < mTiffStream.getReadByteCount() + size) {
                    ...
                    if (event instanceof ImageEvent) {
                        ...
                        System.out.println("Thumbnail overlaps value for tag: \n" + tag.toString());
                        Entry<Integer, Object> entry = mCorrespondingEvent.pollFirstEntry();
                        System.out.println("Invalid thumbnail offset: " + entry.getKey());
                    } else {
                        // Tag value overlaps another tag, shorten count
                        if (event instanceof IfdEvent) {
                            System.out.println("Ifd " + ((IfdEvent) event).ifd + " overlaps value for tag: \n" + tag.toString());
                        } else if (event instanceof ExifTagEvent) {
                            System.out.println("Tag value for tag: \n" + ((ExifTagEvent) event).tag.toString() + " overlaps value for tag: \n" + tag.toString());
                        }
                        size = mCorrespondingEvent.firstEntry().getKey() - mTiffStream.getReadByteCount();
                        System.out.println("Invalid size of tag: \n" + tag.toString() + " setting count to: " + size);
